...
//-woah-	struct rte_eth_conf port_conf;
//-woah-	struct rte_eth_dev_info dev_info;
//-woah-
//-woah-	const uint16_t rx_rings = 1, tx_rings = 0;
//-woah-	uint16_t nb_rxd = RX_RING_SIZE;
//-woah-	uint16_t nb_txd = TX_RING_SIZE;
//-woah-
//-woah-	printf("Initialising port %d\n", port_id);
//-woah-	if (!rte_eth_dev_is_valid_port(port_id))
//-woah-		return -1;
//-woah-
//-woah-	memset(&port_conf, 0, sizeof(struct rte_eth_conf));
//-woah-
//-woah-	int retval = rte_eth_dev_info_get(port_id, &dev_info);
//-woah-	if (retval != 0) {
//-woah-		RTE_LOG(WARNING, EAL, "Error during getting device (port %u) info: %s\n", port_id, strerror(-retval));
//-woah-		return retval;
//-woah-	}
//-woah-
//-woah-	retval = rte_eth_dev_configure(port_id, rx_rings, tx_rings, &port_conf);
//-woah-	if (retval != 0) {
//-woah-		return retval;
//-woah-	}
//-woah-	rte_pmd_qdma_get_device(port_id);
//-woah-
//-woah-	retval = rte_eth_dev_adjust_nb_rx_tx_desc(port_id, &nb_rxd, &nb_txd);
//-woah-	if (retval != 0)
//-woah-		return retval;
//-woah-
//-woah-	char mbuf_name[20];
//-woah-	snprintf(mbuf_name, 20, "%s%d", "MBUF_POOL", port_id);
//-woah-	struct rte_mempool *mbuf_pool = rte_pktmbuf_pool_create(mbuf_name, NUM_MBUFS, MBUF_CACHE_SIZE, 0, MBUF_DATAROOM_SIZE, rte_socket_id());
//-woah-
//-woah-	for ((int q = 0); (q < rx_rings); q++) {
//-woah-		retval = rte_eth_rx_queue_setup(port_id, q, nb_rxd, rte_socket_id(), NULL, mbuf_pool);
//-woah-		if (retval != 0)
//-woah-			return retval;
//-woah-	}
//-woah-
//-woah-	retval = rte_eth_dev_start(port_id);
//-woah-	if (retval < 0) {
//-woah-		RTE_LOG(ERR, EAL, "Error during getting device (port %u) info: %s\n", port_id, strerror(-retval));
//-woah-		return retval;
